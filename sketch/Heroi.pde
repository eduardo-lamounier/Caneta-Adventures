public class Heroi extends Personagem {
  ...
  public void escolher_habilidade() {
    ...
    color cor_fundo_fora_cooldown = color(#5A5A58);
    color cor_fundo_em_cooldown = color(#323230);
    color cor_texto_fora_cooldown = color(#FFFFFF);
    color cor_texto_em_cooldown = color(#aaaaaa);
    ...
    for(int i = 0; i < 3; i++) {
      botoes_selecao_habilidade[i] = new BotaoTexto(
        ...
        comprimento,
        altura,
        (habilidades[i].get_cooldown() > 0 ? ("["+habilidades[i].get_cooldown()+"] ") : "")
        ...
        habilidades[i].get_cooldown() > 0 ? cor_fundo_em_cooldown : cor_fundo_fora_cooldown
      );
    }
  }
